...
class Meta:
    ...
    def serialize(self):
        ...
        print("freelistpage", self.freelist_page)
        buf = self.freelist_page.to_bytes(
            PAGE_NUM_SIZE, byteorder='little', signed='False')
        return buf

    def deserialize(self, buf):
        print(len(buf))
        # if len(buf)%PAGE_NUM_SIZE==0:
        #     raise ValueError(
        #         "Size of the buffer must be of {PAGE_NUM_SIZE} size")
        self.freelist_page = int.from_bytes(
            buf[:PAGE_NUM_SIZE], byteorder='little', signed='False')
